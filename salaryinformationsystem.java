		...
		if (a == 3) {
			...
			if (salary > 50000) {
				...
				if (year <= 5) {
					...
					double maximumloanamount = salary * 0.60;
					double n = year * 12;
					double monthlyintrate = 0.15 / 12;

					double lamount = maximumloanamount * (1 - (1 / Math.pow(1 + monthlyintrate, n))) / monthlyintrate;

					System.out.print("\t\t\tYou can get Loan Amount : " + Math.round(lamount / 1000) * 1000);

				} else {
					System.out.print("\t\t\tyou enter the wrong year.your maximum number of the year is 5");
